...
if(isset($_POST['submit'])) {
    ...
    // Check for duplicate NIC (excluding current resident)
    $check_nic = $conn->prepare("SELECT id FROM residents WHERE nic = ? AND id != ?");
    $check_nic->bind_param("si", $nic, $id);
    $check_nic->execute();
    if ($check_nic->fetch()) {
    ...
    if(empty($errors)) {
        ...
        try {
            $stmt = $conn->prepare("UPDATE residents SET full_name=?, dob=?, nic=?, phone=?, address=?, email=?, occupation=?, gender=? WHERE id=?");
            $stmt->bind_param("ssssssssi", $name, $dob, $nic, $num, $address, $email, $occupation, $gender, $id);
...
<head>
    ...
    <title>Update Resident - Grama Niladhari</title>
    ...
    <div class="main-content">
        <div class="header">
            <div>
                ...
                <h1><i class="fas fa-user-edit me-2"></i>Update Resident</h1>
            </div>
            ...
            <form method="post" class="needs-validation" novalidate>
                <div class="row">
                    <div class="col-md-4 mb-4">
                        <div class="avatar-preview">
                            <span id="avatar-text"><?= strtoupper(substr($resident['full_name'], 0, 1)) ?></span>
                        </div>
                        <div class="text-center">
                            <small class="text-muted">Avatar generated from name</small>
                        </div>
                    </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" name="InputFullName" 
                                           value="<?= htmlspecialchars($resident['full_name'] ?? '') ?>" 
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="date" class="form-control" name="InputDOB" 
                                           value="<?= htmlspecialchars($resident['dob'] ?? '') ?>" 
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" name="InputNIC" 
                                           value="<?= htmlspecialchars($resident['nic'] ?? '') ?>" 
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" name="InputGender" required>
                                        ...
                                        <option value="Other" <?= (isset($resident['gender']) && $resident['gender'] === 'Other') ? 'selected' : '' ?>>Other</option>
                                    </select>
                ...
                <div class="form-section">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="InputAddress" rows="3" required><?= htmlspecialchars($resident['address'] ?? '') ?></textarea>
    ...
    <script>
        ...
        document.querySelector('[name="InputFullName"]').addEventListener('input', function() {
            ...
            if (name.length > 0) {
                ...
                avatarText.textContent = '<?= strtoupper(substr($resident['full_name'], 0, 1)) ?>';
            }
        });
